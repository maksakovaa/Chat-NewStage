...
void menuPrivateSendMsg()
{
	...
	{
		consoleClear();
		AllMsgBase->printPers(UserBase->getUsrName(temp), UserBase->getUsrName(userId));
...
#endif
			AllMsgBase->sendMsg(UserBase->getUsrName(temp), UserBase->getUsrName(userId), msgText);
			break;
		}
		case 4:
		{
...
void menuMainChat()
{
	...
	while (choice == 1)
	{
		...
		case 1:
		{
			...
			msgText = cp1251toUtf8(msgText.data());
#endif
			AllMsgBase->sendMsg(UserBase->getUsrName(userId), "ALL",  msgText);
			break;
		}
		case 2:
		{
