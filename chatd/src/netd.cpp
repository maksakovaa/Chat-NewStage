...
net::~net()
{
    ...
    if (iResult == SOCKET_ERROR) {
        cout << "[" << curDateTime() << "] " << "ERROR: shutdown failed with error:" << WSAGetLastError() << endl;
...
void net::netGateway()
{
    ...
    while (true)
    {
        ...
        if (iResult == SOCKET_ERROR)
        {
            cout << "[" << curDateTime() << "] " << "ERROR: bind failed with error:" << WSAGetLastError() << endl;
            freeaddrinfo(result);
...
void net::regMSG()
{
    ...
    AllMsgBase->sendMsg(newMsg);
    cout << "[" << curDateTime() << "] " << "Message " << newMsg.msgText << " send" << endl;
}
